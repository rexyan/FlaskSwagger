...
from swagger.doc import route_specs, serialize_schema, definitions
...
def flask_swagger(app):
    ...
    _swagger_json["info"] = {
        ...
        "title": getattr(app, "name", "API"),
        "description": getattr(app.config, "API_DESCRIPTION", ""),
        "termsOfService": getattr(app.config, "API_TERMS_OF_SERVICE", "HuiFu"),
    ...
    _swagger_json["schemes"] = getattr(app.config, "PREFERRED_URL_SCHEME", ["http"])
    ...
    # Authorization
    _swagger_json["securityDefinitions"] = getattr(app.config, "API_SECURITY_DEFINITIONS", None)
    _swagger_json["security"] = getattr(app.config, "API_SECURITY", None)

    # Tags
    _swagger_json["tags"] = [{"name": "test"}]
    ...
    for rule in app.url_map._rules:
        ...
        _swagger_json["paths"][rule.rule] = {
            _method: {
                ...
                "tags": [SwaggerHelper.app_name(app)],
